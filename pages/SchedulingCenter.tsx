...
const SchedulingCenter: React.FC<SchedulingCenterProps> = ({ type }) => {
  ...
  const filteredSchedules = schedules.filter(s => {
    ...
    const matchesSearch = s.date.includes(searchTerm) || (s.examinerIds && Array.isArray(s.examinerIds) && (s.examinerIds as string[]).some(id => getExaminerName(id).toLowerCase().includes(searchTerm.toLowerCase())));
        ...
        <div className="space-y-6 animate-fadeIn pb-20">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <button onClick={() => window.print()} className="flex items-center gap-2 px-6 py-2.5 bg-slate-900 text-white rounded-xl hover:bg-black shadow-lg text-sm font-black uppercase tracking-widest"><Printer className="h-4 w-4" /> Imprimir</button>
            ...
            {/* HEADER DA BANCA SELECIONADA */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            <h2 className="text-3xl font-black text-gray-900 tracking-tight">{formatDateDisplay(selectedSchedule.date)}</h2>
                        ...
                        <div className="flex flex-wrap gap-6 text-sm font-medium text-gray-500">
                            ...
                            <span className="flex items-center gap-2"><User className="h-4 w-4" /> {selectedSchedule.examinerIds && Array.isArray(selectedSchedule.examinerIds) && selectedSchedule.examinerIds.map(id => getExaminerName(id as string)).join(', ')}</span>
                        </div>
                    </div>
                    <div className="text-right">
                        <span className="text-xs font-bold text-gray-400 uppercase block">Total Agendados</span>
                        <span className="text-3xl font-black text-blue-600">{scheduledStudents.length}</span>
                    </div>
                </div>
            </div>
                        ...
                        <div key={cat} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                                <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded-lg ${cat === 'A' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600'}`}>
                                        {cat === 'A' ? <Bike className="h-5 w-5" /> : <Car className="h-5 w-5" />}
                                    </div>
                                    <h3 className="text-lg font-black uppercase text-gray-800">Categoria {cat}</h3>
                                </div>
                                <span className="text-xs font-bold text-gray-400 bg-white px-3 py-1 rounded-full border">{students.length} candidatos</span>
                            </div>

                            <div className="divide-y divide-gray-100">
                                {students.map((req, idx) => (
                                    <div key={req.id} className="p-4 flex flex-col sm:flex-row items-center justify-between gap-4 hover:bg-gray-50 transition-colors">
                                        <div className="flex items-center gap-4 flex-1">
                                            <div className="h-8 w-8 rounded-full bg-slate-100 flex items-center justify-center font-bold text-xs text-slate-500">{idx + 1}</div>
                                            <div>
                                                <div className="font-bold text-slate-800 uppercase">{req.socialName || req.studentName}</div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            >
                                                {req.attendanceConfirmed ? <CheckCircle2 className="h-3 w-3" /> : <CheckCircle className="h-3 w-3" />}
                                                {req.attendanceConfirmed ? 'Confirmado' : 'Confirmar'}
                                            </button>
                                            
                                            <button onClick={() => handleWhatsApp(req)} className="p-2 text-green-600 hover:bg-green-50 rounded-lg transition-colors border border-transparent hover:border-green-100" title="Enviar WhatsApp">
                                                <MessageCircle className="h-4 w-4" />
                                            </button>
                                            
                                            <button onClick={() => handleRemoveStudent(req.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-transparent hover:border-red-100" title="Remover da Banca">
                                ...
                                {students.length === 0 && (
                                    <div className="p-8 text-center text-gray-400 text-sm">Nenhum candidato agendado para esta categoria.</div>
                                )}
                            </div>
                        </div>
                    );
