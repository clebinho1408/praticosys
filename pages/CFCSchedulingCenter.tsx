...
const CFCSchedulingCenter: React.FC<CFCSchedulingCenterProps> = ({ user }) => {
  ...
  const [activeResultTab, setActiveResultTab] = useState<'A' | 'B' | 'MUDANCA'>('A');
  const [currentResultData, setCurrentResultData] = useState<Partial<BancaResult>>({
    totalSlots: 0,
    usedSlots: 0,
    approved: 0,
    failed: 0,
  ...
  const submitCancel = async () => {
    ...
    try {
      ...
      await api.updateRequest(cancelRequest.id, { 
        ...
        observation: cancelReason ? `Motivo Cancelamento: ${cancelReason}` : 'Cancelado'
      });
        ...
        if (cnhSchedule.active && cnhSchedule.days.includes(selectedDayName)) {
          ...
          for (const slot of slots) {
            ...
            await api.createRequest({
              ...
              scheduledTime: slot.time,
              examinerId: slot.examiner,
              observation: 'Escala Fixa CNH Brasil'
            });
            count++;
          }
        }
  ...
  const handleSubmitNew = async () => {
    ...
    try {
      await api.createRequest({
        ...
        observation: newRequest.observation || (newRequest.requestType === RequestType.FIXA ? 'Escala Fixa' : newRequest.requestType === RequestType.EXTRA ? 'Solicitação Extra' : '')
      });
  ...
  const handleOpenResultsModal = (req: ExamRequest) => {
    setSelectedRequest(req);
    // Find if there's already a result for this schedule and school
    const existingResults = bancaResults.filter(r => r.scheduleId === req.scheduleId && r.schoolId === req.schoolId);
    
    // Determine which categories are available for this school in this schedule
    const schoolRequestsInBanca = requests.filter(r => r.scheduleId === req.scheduleId && r.schoolId === req.schoolId);
    const categories = Array.from(new Set(schoolRequestsInBanca.map(r => r.scheduledCategory || r.intendedCategory).filter(Boolean)));
    
    // Set active tab to the first available category
    let initialTab: 'A' | 'B' | 'MUDANCA' = 'A';
    if (categories.some(c => c?.includes('A'))) initialTab = 'A';
    else if (categories.some(c => c?.includes('B'))) initialTab = 'B';
    else if (categories.some(c => ['C', 'D', 'E'].some(cat => c?.includes(cat)))) initialTab = 'MUDANCA';

    setActiveResultTab(initialTab);

    // Load data for the active tab
    const resultForTab = existingResults.find(r => r.category === initialTab);
    
    if (resultForTab) {
      setCurrentResultData(resultForTab);
    } else {
  ...
  const handleResultTabChange = (tab: 'A' | 'B' | 'MUDANCA') => {
    if (!selectedRequest) return;
    setActiveResultTab(tab);
    const existing = bancaResults.find(r => r.scheduleId === selectedRequest.scheduleId && r.schoolId === selectedRequest.schoolId && r.category === tab);
    if (existing) {
      setCurrentResultData(existing);
    } else {
      setCurrentResultData({
  ...
  const handleSaveResults = async () => {
    ...
    try {
      ...
      const updatedResults = await api.getBancaResults();
      setBancaResults(updatedResults);
                       ...
                       {done.map(req => (
                         <tr key={req.id} className={getRowClass(req)}>
                           ...
                           <td className="px-4 py-3 text-right">
                             <button 
                               onClick={() => handleOpenResultsModal(req)}
          ...
          >
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-black text-slate-800 tracking-tight uppercase">Resultados da Banca</h2>
                  <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mt-0.5">
                    {getSchoolName(selectedRequest.schoolId)} - {formatDate(selectedRequest.scheduledDate)}
                  </p>
                </div>
              </div>
              <button 
            ...
            <div className="flex border-b border-slate-100 bg-slate-50/30">
              {(() => {
                const schoolRequestsInBanca = requests.filter(r => r.scheduleId === selectedRequest.scheduleId && r.schoolId === selectedRequest.schoolId);
                const categories = Array.from(new Set(schoolRequestsInBanca.map(r => r.scheduledCategory || r.intendedCategory).filter(Boolean)));
                
                const tabs: { id: 'A' | 'B' | 'MUDANCA', label: string }[] = [];
                if (categories.some(c => c?.includes('A'))) tabs.push({ id: 'A', label: 'Categoria A' });
                if (categories.some(c => c?.includes('B'))) tabs.push({ id: 'B', label: 'Categoria B' });
                if (categories.some(c => ['C', 'D', 'E'].some(cat => c?.includes(cat)))) tabs.push({ id: 'MUDANCA', label: 'Mudança Categoria' });
            ...
            <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Total Vagas</label>
                  <input 
                    type="number" 
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium"
                    value={currentResultData.totalSlots || 0}
                    onChange={(e) => setCurrentResultData(prev => ({ ...prev, totalSlots: parseInt(e.target.value) || 0 }))}
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Vagas Usadas</label>
                  <input 
                    type="number" 
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium"
                    value={currentResultData.usedSlots || 0}
                    onChange={(e) => setCurrentResultData(prev => ({ ...prev, usedSlots: parseInt(e.target.value) || 0 }))}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider">Apto</label>
                  <input 
                    type="number" 
                    className="w-full px-4 py-2.5 bg-emerald-50 border border-emerald-100 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all font-bold text-emerald-700"
                    value={currentResultData.approved || 0}
                    onChange={(e) => setCurrentResultData(prev => ({ ...prev, approved: parseInt(e.target.value) || 0 }))}
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-red-600 uppercase tracking-wider">Inapto</label>
                  <input 
                    type="number" 
                    className="w-full px-4 py-2.5 bg-red-50 border border-red-100 rounded-xl focus:ring-2 focus:ring-red-500/20 focus:border-red-500 outline-none transition-all font-bold text-red-700"
                    value={currentResultData.failed || 0}
                    onChange={(e) => setCurrentResultData(prev => ({ ...prev, failed: parseInt(e.target.value) || 0 }))}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-amber-600 uppercase tracking-wider">Faltou</label>
                  <input 
                    type="number" 
                    className="w-full px-4 py-2.5 bg-amber-50 border border-amber-100 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all font-bold text-amber-700"
                    value={currentResultData.absent || 0}
                    onChange={(e) => setCurrentResultData(prev => ({ ...prev, absent: parseInt(e.target.value) || 0 }))}
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Cancelado</label>
                  <input 
                    type="number" 
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium"
                    value={currentResultData.cancelled || 0}
                    onChange={(e) => setCurrentResultData(prev => ({ ...prev, cancelled: parseInt(e.target.value) || 0 }))}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button 
                onClick={() => setIsResultsModalOpen(false)}
                className="px-6 py-2.5 text-slate-600 hover:bg-slate-200 rounded-xl font-bold uppercase text-xs tracking-wider transition-all"
              >
                Voltar
              </button>
              <button 
                onClick={handleSaveResults}
                className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold uppercase text-xs tracking-wider shadow-lg shadow-blue-500/20 transition-all"
              >
                Salvar Resultados
              </button>
            </div>
