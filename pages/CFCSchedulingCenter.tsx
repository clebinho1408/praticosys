  ...
  const handleSubmitNew = async () => {
    ...
    if (user.role === UserRole.SCHOOL) {
      if (newRequest.requestType === RequestType.EXTRA) {
        ...
        const monthlyExtras = requests.filter(r => {
        ...
        if (monthlyExtras.length >= 3) {
          alert('Limite de 3 solicitações extras por mês atingido.');
          return;
        }
      }

      if (!awareOfLimits) {
        alert('Você precisa confirmar que está ciente dos limites de vagas por categoria.');
        return;
      }
    }
                ...
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-100 space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm text-blue-800 leading-relaxed">
                      <p className="font-bold mb-1">Aviso de Limite de Vagas:</p>
                      <p>
                        Serão oferecidas no máximo <strong>{systemSettings?.defaultMaxSlotsA || 10}</strong> vagas para categoria <strong>A</strong>, 
                        <strong> {systemSettings?.defaultMaxSlotsB || 10}</strong> vagas para categoria <strong>B</strong>, 
                        e no máximo <strong>{systemSettings?.defaultMaxSlotsMudanca || 10}</strong> vagas para as categorias <strong>C, D e E juntas</strong>.
                      </p>
                    </div>
                  </div>
                  <label className="flex items-center gap-3 cursor-pointer group p-2 hover:bg-blue-100/50 rounded transition-colors">
                    <input 
                      type="checkbox" 
                      className="h-5 w-5 rounded border-blue-300 text-blue-600 focus:ring-blue-500 transition-all"
                      checked={awareOfLimits}
                      onChange={e => setAwareOfLimits(e.target.checked)}
                    />
                    <span className="text-sm font-bold text-blue-900 group-hover:text-blue-700 transition-colors">
                      Estou ciente e concordo com os limites de vagas informados.
                    </span>
                  </label>
                </div>
              )}
