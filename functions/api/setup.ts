...
export const onRequestPost: PagesFunction<{ DATABASE_URL: string }> = async ({ env }) => {
  try {
    ...
    const tables = [
      ...
      sql`CREATE TABLE IF NOT EXISTS system_settings (id integer PRIMARY KEY DEFAULT 1, agency_name text DEFAULT 'DETRAN', agency_address text, maintenance_mode boolean DEFAULT false, min_days_scheduling integer DEFAULT 2, max_daily_slots integer DEFAULT 50, default_max_slots_a integer DEFAULT 10, default_max_slots_b integer DEFAULT 10)`,
