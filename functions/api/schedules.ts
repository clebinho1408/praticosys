...
export const onRequest: PagesFunction<{ DATABASE_URL: string }> = async ({ request, env }) => {
  try {
    ...
    if (method === 'GET') {
      ...
      for (const s of schedules) {
        ...
        if (calc !== s.status) {
          ...
          // Remoção apenas via botão manual.
    ...
    if (method === 'POST') {
      ...
      const last = await db.select({ code: examSchedules.code }).from(examSchedules).where(isNotNull(examSchedules.code)).orderBy(desc(examSchedules.createdAt)).limit(1);
      let nextCode = 'B1000';
      if (last.length > 0 && last[0].code) {
        const n = parseInt(last[0].code.replace('B', ''), 10);
        if (!isNaN(n)) nextCode = `B${n + 1}`;
      }
      const newItem = await db.insert(examSchedules).values({ id: randomUUID(), code: nextCode, status: initialStatus, ...body, date: cleanDate }).returning();
    ...
    if (method === 'PUT') {
      ...
      if (updates.date || updates.time) {
        await db.update(examRequests).set({ scheduledDate: updates.date, scheduledTime: updates.time }).where(eq(examRequests.scheduleId, id));
      }
      return json(current);
    }
